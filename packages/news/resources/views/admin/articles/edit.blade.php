    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Cập nhật thông tin bài viết</p>
            </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="excerpt" id="article-excerpt" rows="3"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black"
                                placeholder="Tóm tắt ngắn gọn về bài viết">{{ old('excerpt', $article->excerpt) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="content" id="article-content" rows="15"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black">{{ old('content', $article->content) }}</textarea>
                            @error('content')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div id="featured-image-preview" class="mt-2">
                            @if (old('featured_image', $article->featured_image))
                                <img src="{{ old('featured_image', $article->featured_image) }}"
                ...
                <x-admin.card title="Thẻ tag">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="tags_input" id="article-tags"
                            ...
                            <input type="hidden" name="tags" id="tags-json"
                                value="{{ old('tags', is_array($article->tags) ? json_encode($article->tags) : '[]') }}">
                        </div>
                    </div>
                </x-admin.card>
                ...
                <x-admin.card title="SEO">
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">Để trống sẽ sử dụng tiêu đề bài viết. Tối đa 60 ký tự.</p>
                            @error('meta_title')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="meta_description" id="meta-description" rows="3"
                                ...
                                placeholder="Mô tả SEO (tối đa 160 ký tự)">{{ old('meta_description', $article->meta_description) }}</textarea>
                            <p class="mt-1 text-xs text-gray-500">Để trống sẽ sử dụng tóm tắt bài viết. Tối đa 160 ký tự.</p>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">Phân cách các từ khóa bằng dấu phẩy (ví dụ: tin tức, blog, seo)</p>
                            @error('meta_keywords')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" id="article-status"
                                ...
                                <option value="archived"
                                    {{ old('status', $article->status) === 'archived' ? 'selected' : '' }}>Đã lưu trữ
                                </option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
