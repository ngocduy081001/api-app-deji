<x-admin-layout>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="short_description" id="short-description" rows="3"
                                class="w-full rounded-md  shadow-sm focus:border-black focus:ring-black" placeholder="Mô tả ngắn gọn về sản phẩm">{{ old('short_description', $product->short_description) }}</textarea>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Mô tả chi tiết</label>
                            <textarea name="description" id="description" rows="10"
                                class="w-full rounded-md  shadow-sm focus:border-black focus:ring-black" placeholder="Mô tả đầy đủ về sản phẩm">{{ old('description', $product->description) }}</textarea>
                        </div>
                    </div>
                </x-admin.card>
                ...
                <x-admin.card title="Thông số kỹ thuật">
                    <div x-data="technicalSpecs({{ json_encode($product->meta_data['specifications'] ?? []) }})" class="space-y-4">
            ...
            <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="price" id="price-input"
                                        ...
                                        data-original-value="{{ old('price', $product->price) }}" required
                                        class="w-full pl-7 rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black price-input"
                                        placeholder="0">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <span class="absolute left-3 top-2 text-gray-500">₫</span>
                                    <input type="text" name="sale_price" id="sale-price-input"
                                        value="{{ old('sale_price', $product->sale_price ? number_format($product->sale_price, 0, ',', '.') : '') }}"
                                        data-original-value="{{ old('sale_price', $product->sale_price) }}"
                                        class="w-full pl-7 rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black price-input"
                                        placeholder="0">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <span class="absolute left-3 top-2 text-gray-500">₫</span>
                                    <input type="text" name="replacement_price" id="replacement-price-input"
                                        value="{{ old('replacement_price', isset($product->meta_data['replacement_price']) && $product->meta_data['replacement_price'] ? number_format($product->meta_data['replacement_price'], 0, ',', '.') : '') }}"
                                        data-original-value="{{ old('replacement_price', $product->meta_data['replacement_price'] ?? '') }}"
                                        class="w-full pl-7 rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black price-input"
                                        placeholder="0">
                                </div>
                            </div>
                ...
                <x-admin.card title="Hình ảnh">
                    <div x-data="imageGallery({{ json_encode($product->featured_image) }}, {{ json_encode($product->images ?? []) }})" class="space-y-4">
                        <!-- Featured Image -->
                        <div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function initPriceFormatting() {
                ...
                priceInputs.forEach(input => {
                    ...
                    input.addEventListener('input', function(e) {
                        ...
                        const unformatted = unformatPrice(oldValue);
                    ...
                    input.addEventListener('paste', function(e) {
                        ...
                        const unformatted = unformatPrice(pastedText);
                        this.value = formatPrice(unformatted);
                    });
                });

                // Before form submit, unformat all price values
                const form = document.querySelector('form[method="POST"]');
                if (form) {
                    form.addEventListener('submit', function(e) {
                        priceInputs.forEach(input => {
                            const unformatted = unformatPrice(input.value);
                            // Update the input value to unformatted number before submit
                            input.value = unformatted || '0';
                        });
                    });
                }
            }
            ...
            // Initialize price formatting immediately when DOM is ready
            ...
            function imageGallery(initialFeaturedImage = null, initialGalleryImages = []) {


                // Normalize initial values
                const featuredImg = initialFeaturedImage && initialFeaturedImage !== 'null' ? initialFeaturedImage : null;
                const galleryImgs = Array.isArray(initialGalleryImages) ? initialGalleryImages : [];

                return {
                    featuredImage: featuredImg ? featuredImg : null,
                    galleryImages: galleryImgs.map(img => {
                        const imgPath = typeof img === 'string' ? img : (img.path || img);
                        return {
                            url: imgPath,
                            path: imgPath
                        };
                    }),
                    galleryPaths: galleryImgs.map(img => typeof img === 'string' ? img : (img.path ||
                        img)), // Store paths for gallery images

                    init() {
                        // Set featured image input value if exists
                        if (this.featuredImage && featuredImg) {
                            const featuredInput = document.getElementById('featured-image-input');
                            if (featuredInput) {
                                featuredInput.value = featuredImg;
                            }
                        }
                        ...
                        if (this.galleryImages.length > 0) {
                            ...
                            if (galleryInput) {
                                this.galleryImages.forEach(img => {
                                    ...
                                    galleryInput.appendChild(hiddenInput);
                                });
                            }
                        }
            ...
            function technicalSpecs(initialSpecs = []) {
                return {
